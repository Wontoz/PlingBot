...
public class CouponEvaluator
{
    ...
    public static string? GetCurrentSymbol(TipsMatch tip)
    {
        if (!string.IsNullOrWhiteSpace(tip.Outcome))
            return tip.Outcome;

        if (tip.Match != null)
            return tip.Match.Status.Short.Equals("PST", StringComparison.OrdinalIgnoreCase) ? null : tip.Match.Symbol;
